...
public class DriveServiceImpl implements DriveService {
    ...
    public void downloadFile() {
        String fileId = "0B_3L9MP_EcpkdTF6SnN3bWI4OHRVRU0xbHFNV0VJdnk2TkxR";
        try {
            ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
            driveService.files().get(fileId).executeMediaAndDownloadTo(outputStream);
            try(OutputStream outputStream2 = new FileOutputStream(applicationConfig.getTemporaryFolder() + "/yourFile.pdf")) {
                outputStream.writeTo(outputStream2);
            }
            logger.debug("File ID is download: " + fileId);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
